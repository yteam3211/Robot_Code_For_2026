...
public class devButoon {
    ...
    public static Rotation2d findAngle(){
        double x = AllianceFlipUtil.applyX(FieldConstants.Hub.innerCenterPoint.getX()) - Drive.getInsatnce().getPose().getX();
        double y = AllianceFlipUtil.applyY(FieldConstants.Hub.innerCenterPoint.getY()) - Drive.getInsatnce().getPose().getY();
    ...
    private static void sysidAll(Controler controller) {
        ...
        // controller.swerveController.square().whileTrue(IntakePitch.getInstance().setStateCommand(IntakePitchState.Open));
        // controller.swerveController.square().whileFalse(IntakePitch.getInstance().setStateCommand(IntakePitchState.colse));
        ...
        // controller.swerveController.triangle().onTrue(IntakePitch.getInstance().sysidQuasistatic(Direction.kForward));
        // controller.swerveController.square().onTrue(IntakePitch.getInstance().sysidQuasistatic(Direction.kReverse));
        // controller.swerveController.circle().onTrue(IntakePitch.getInstance().sysidDynamic(Direction.kForward));
        // controller.swerveController.cross().onTrue(IntakePitch.getInstance().sysidDynamic(Direction.kReverse));

        controller.swerveController.triangle().whileTrue(DriveCommands.joystickDriveAtAngle(Drive.getInsatnce(), ()->0, ()->0, ()-> findAngle()));
        controller.swerveController.square().onTrue(Commands.runOnce(()-> spwanFuel()));
    }
