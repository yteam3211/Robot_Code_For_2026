...
public class moveToRotation extends Command {
  ...
  private static PIDController rotController = new PIDController(5, 0, 0.2);
  ...
  @Override
  public void execute() {
        Logger.recordOutput("SuperStruc/degreeToHubErorr", 
        Math.max(Drive.getInsatnce().getRotation().getDegrees(), RotTarget.getDegrees()) - 
        Math.min(Drive.getInsatnce().getRotation().getDegrees(), RotTarget.getDegrees()));
        double rotOut = rotController.calculate(Drive.getInsatnce().getRotation().getRadians(), RotTarget.getRadians());
        ChassisSpeeds chassisSpeeds = ChassisSpeeds.fromFieldRelativeSpeeds(0, 0, rotOut, Drive.getInsatnce().getRotation());
        Drive.getInsatnce().runVelocity(chassisSpeeds);
  }
  ...
  public static Command setPid(double p, double i, double d){
    return Commands.runOnce(()-> rotController = new PIDController(p, i, d));
  } 
}
